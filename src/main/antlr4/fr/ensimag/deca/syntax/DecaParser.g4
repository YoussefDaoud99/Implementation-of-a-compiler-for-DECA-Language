...
@init   {
	AbstractInitialization initialiser;
        }
    : i=ident {
    	initialiser = new NoInitialization();
        }
      (EQUALS e=expr {
      	initialiser = new Initialization($e.tree);
      	setLocation(initialiser, $e.start);
        }
      )? {
      	$tree = new DeclVar($t, $i.tree, initialiser);
...
@init {
    ...
    : if1=IF OPARENT condition=expr CPARENT OBRACE li_if=list_inst CBRACE {
      ...
      (ELSE elsif=IF OPARENT elsif_cond=expr CPARENT OBRACE elsif_li=list_inst CBRACE {
            elsifBranch = new IfThenElse($elsif_cond.tree, $elsif_li.tree, new ListInst());
            setLocation(elsifBranch, $ELSE);
            elseBranch.add(elsifBranch);
            elseBranch = elsifBranch.getElseBranch();
        }
      )*
      (ELSE OBRACE li_else=list_inst CBRACE {
            if (elsifBranch == null)
                ...
                elsifBranch.setElseBranch($li_else.tree);
        }
      )?
    ;
...
class_decl returns[AbstractDeclClass tree]
    : CLASS name=ident superclass=class_extension OBRACE class_body CBRACE {
    	$tree = new DeclClass($name.tree, $superclass.tree, $class_body.list_fields, $class_body.list_methods);
...
class_body returns[ListDeclFields list_fields, ListDeclMethods list_methods]
@init {
        $list_fields = new ListDeclFields();
        $list_methods = new ListDeclMethods();
}
    : (m=decl_method {
    	$list_methods.add($m.tree);
    	}
      | decl_field_set[$list_fields]
      )*
    ;

decl_field_set[ListDeclFields list_fields]
    : v=visibility t=type list_decl_field[$v.tree, $t.tree, $list_fields]
      SEMI
    ;
...
list_decl_field[Visibility v, AbstractIdentifier t, ListDeclFields list_fields]
: dv1=decl_field[$v, $t] {
            $list_fields.add($dv1.tree);
    }
        (COMMA dv2=decl_field[$v, $t] {
            $list_fields.add($dv2.tree);
        }
      )*
    ;
...
@init {
    ...
    : i=ident {
    	assert($i.tree != null);
    	initialiser = new NoInitialization();
    	$tree = new DeclField($v, $t, $i.tree, initialiser);	
        }
      (EQUALS e=expr {
        assert($e.tree != null);
      	initialiser = new Initialization($e.tree);
...
@init {
    ...
    : type ident OPARENT params=list_params CPARENT (block {
      ...
      | ASM OPARENT code=multi_line_string CPARENT SEMI {
        ...
        methodBody = new MethodAsmBody(asmCode);
